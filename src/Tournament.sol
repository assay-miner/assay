...
contract Tournament {
    ...
    function vaultUISchema() external pure returns (VaultUISchema memory schema) {
        ...
        m = schema.methods[6];
        m.name = "postTask";
        m.description = unicode"Publish a task and escrow its pot (curator) / 发布任务并托管奖池(策展方)";
        ...
        m.inputs[2] = FieldDescriptor("baselineGas", "uint256", unicode"Reference cost to beat / 要跑赢的参考成本", 0);
